...
# -----------------------------
# 3번 정답
# 같은 '먹-'이 두 번 나오므로 Counter로 개수까지 비교
# -----------------------------
Q3_ANSWERS = {
...
def split_short_answers(text: str):
    """
    3번용.
    학생이 쉼표, 슬래시, 가운데점, 줄바꿈 중 무엇을 써도 형태소 항목으로 분리.
    단, 붙임표는 삭제하거나 보정하지 않음 → 학생이 정확히 써야 정답.
    """
    if not text:
        return []
    parts = re.split(r"[,/·|\n]+", text)
        ...
        for category, user_text in user_map.items():
            if same_items_ignore_order(user_text, Q3_ANSWERS[category]):
                ...
                st.warning(
                    ...
                    "형태소의 누락·중복 또는 붙임표 위치가 틀릴 수 있습니다."
                )
